...
#cnn_model = load_model("cnnVamide2012.h5")prot
cnn_model = load_model("NitroTyrCNNModel201226.h5")
...
class PredForm(Form):
    ...
    #sequence = TextAreaField(u'Enter Protien Sequence (Fasta or Text Format))',[validators.DataRequired()])
    sequence = TextAreaField(u'Enter Protien Sequence (Text Format))')
    #print (sequence)
...
def pred():
    ...
    if request.method == 'POST':
        ...
        sample_list, idx_list = protutil.samplesfromProtein(amino_acid, sequence)
        #***********************************************************************
        #sess = tf.Session()
        #K.set_session(sess)
        #try:
        ...
        #sess.close()
        tmp= np.nonzero(y)[0]  # returns the index of nonzero (postive) labels in predicted y
